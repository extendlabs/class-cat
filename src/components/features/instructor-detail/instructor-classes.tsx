...
function ClassCard({ cls }: { cls: Activity }) {
  return (
    <Link
...
export function InstructorClasses({ classes }: { classes: Activity[] }) {
  const freelance = classes.filter((c) => !c.businessId);
  const businessGroups = new Map<string, { name: string; classes: Activity[] }>();

  for (const cls of classes) {
    if (cls.businessId) {
      const existing = businessGroups.get(cls.businessId);
      if (existing) {
        existing.classes.push(cls);
      } else {
        businessGroups.set(cls.businessId, {
          name: cls.provider.name,
          classes: [cls],
        });
      }
    }
  ...
  const hasMultipleSources = freelance.length > 0 && businessGroups.size > 0;
      ...
      <div>
        ...
        {!hasMultipleSources ? (
          ...
          <div className="space-y-10">
            ...
            {Array.from(businessGroups.entries()).map(([bizId, group]) => (
              <div key={bizId}>
                <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">
                  At {group.name}
                </h4>
